...
function cot_select_location($country = '', $region = 0, $city = 0, $userdefault = false)
{
    ...
    if(!$cfg['plugin']['locationselector']['hide_country']) {
      ...
      $country_selectbox = cot_inputbox('hidden', 'country', $country);
    }

		$region = (empty($country)) ? 0 : $region;
		$regions = (!empty($country)) ? cot_getregions($country) : array();
		$regions = array(0 => ($cfg['plugin']['locationselector']['hide_city'] && $cfg['plugin']['locationselector']['hide_city_chg_title'] ? $L['select_city'] : $L['select_region'])) + $regions;
		$disabled = (empty($country)) ? 'disabled="disabled" ' : '';
    ...
    if(!$cfg['plugin']['locationselector']['hide_city']) {
      ...
      $city_selectbox = cot_inputbox('hidden', 'city', $city);
    }

		...
		<div class="uk-margin uk-grid-match uk-grid-medium uk-child-width-1-2@xl" uk-grid>
			...
			<div class="uk-inline thumbnail uk-cover-container">
				<div data-att-display="all">
					{PRD_ROW_ID|att_display('folio',$this,'','attacher.display.foliolist','images',1)}
				</div>
			</div>
			<!-- ELSE -->
			<div class="uk-inline thumbnail uk-cover-container">
							    ...
							    <div class="uk-card-badge uk-card-badge-folio uk-label-folio">
									...
									<div class="uk-box-shadow-large uk-border-rounded" uk-dropdown="mode: click; animation: uk-animation-slide-left;">
										<ul class="uk-nav uk-dropdown-nav uk-list uk-list-divider">
											...
											<li><a class="uk-link-text" href="{PRD_ROW_ID|cot_url('folio','m=edit&id='$this)}"><span class="uk-text-danger" uk-icon="icon: file-edit; ratio: 1.2"></span><span class="uk-margin-small-left uk-text-uppercase uk-text-bold uk-text-middle">{PHP.L.Edit}</span></a></li>
											<!-- ENDIF -->
											<li><a class="uk-link-text" href="{PRD_ROW_URL}" title="{PRD_ROW_SHORTTITLE}"><span class="uk-text-primary" uk-icon="icon: more; ratio: 1.2"></span><span class="uk-margin-small-left uk-text-uppercase uk-text-middle">{PHP.L.open_more}</span></a></li>
											<li><a class="uk-link-text" href="{PHP.R.Folio_How_Add}" title="{PRD_ROW_SHORTTITLE}"><span class="uk-text-primary" uk-icon="icon: question; ratio: 1.2"></span><span class="uk-margin-small-left uk-text-uppercase uk-text-middle">Как добавить свои достижения</span></a></li>
										</ul>
									</div>
								</div>
					<h3 class="uk-heading-bullet uk-margin-remove-top uk-text-truncate"><a class="uk-link-text" href="{PRD_ROW_URL}" title="{PRD_ROW_SHORTTITLE}"><span>{PRD_ROW_SHORTTITLE}</span></a></h3>
					<h5 class="uk-heading-line uk-margin-remove uk-text-center uk-link-text"><span>{PRD_ROW_CATTITLE}</span></h5>
						...
						<div class="uk-grid-small uk-flex-middle" uk-grid>
							...
							<div class="uk-width-expand">
								...
								<h3 class="uk-card-title uk-margin-remove uk-text-truncate">
									<a class="uk-link-text" href="{PRD_ROW_OWNER_DETAILSLINK}"><span class="uk-text-warning uk-margin-small-right">{PRD_ROW_OWNER_FIRST_SECOND_NAME}</span></a>
								</h3>
								<!-- ENDIF -->
								<h4 class="uk-margin-remove"><a class=" uk-margin-remove uk-link-text" href="{PRD_ROW_OWNER_DETAILSLINK}">[{PRD_ROW_OWNER_NICKNAME}]</a></h4>
							</div>
							<!-- UIkit CARD BODY -->
							<div class="uk-card-body uk-margin-remove-top">
								...
								<div class="uk-grid-small uk-flex uk-flex-center uk-flex-middle" uk-grid>
									<div>
								    <!-- IF {PRD_ROW_COST} > 0 -->
										<a class="uk-button uk-button-success" href="{PRD_ROW_URL}"><span uk-icon="icon: credit-card"></span><span class="uk-margin-small-left uk-text-middle uk-text-f5">{PRD_ROW_COST} {PHP.cfg.payments.valuta}</span></a>
									<!-- ENDIF -->
									</div>
...
<div class="uk-grid-small uk-grid-divider uk-child-expand@s uk-flex-center uk-text-center" uk-grid>
    <div>
        <div uk-tooltip="Количество просмотров"><span class="uk-icon uk-text-warning" uk-icon="icon: users; ratio: 1.2"></span><span class="uk-text-bold uk-link-text uk-margin-small-left uk-text-middle">{PRD_ROW_COUNT}</span></div>
    </div>
    <div>
        <div uk-tooltip="Всего прикрепленно файлов / из них картинок / другие для бесплатного скачивания"><span class="uk-margin-small-left uk-icon uk-text-warning" uk-icon="icon: image; ratio: 1.2"></span><span class="uk-text-bold uk-link-text uk-margin-small-left uk-text-middle">{PRD_ROW_ID|att_count('folio',$this)}/{PRD_ROW_ID|att_count('folio',$this,'','images')}/{PRD_ROW_ID|att_count('folio',$this,'','files')}</span></div>
    </div>
    <div class="uk-visible@m">
        <div uk-tooltip="Последнее обновление"><span class="uk-margin-small-left uk-icon uk-text-warning" uk-icon="icon: history; ratio: 1.2"></span><span class="uk-text-bold uk-link-text uk-margin-small-left uk-text-middle">{PHP.item.item_update|date('d.m.Y', $this)}</span></div>
    </div>
    <div>
        <div uk-tooltip="Комментариев всего"><span class="uk-margin-small-left uk-icon uk-text-warning" uk-icon="icon: comments; ratio: 1.2"></span><span class="uk-text-bold uk-link-text uk-margin-small-left uk-text-middle">{LIST_ROW_COMMENTS}</span></div>
						...
						<a class="uk-hidden-hover folio-link-more uk-animation-slide-right" href="{PRD_ROW_URL}"><span uk-icon="icon: link; ratio: 3"></span></a>
					</div>
				</div>
			</div>
			<!-- END: PRD_ROWS -->
	...
	<!-- ELSE -->
		<div class="uk-card uk-card-body uk-border-rounded uk-background-default uk-animation-scale-up">
			...
			<ul class="uk-list uk-list-divider">
				<li><span class="uk-button uk-button-default">ПРОДАВЦУ:</span></li>
				<li>Вы можете стать первым продавцом в этом отделе <span class="uk-text-bold">онлайн маркетплейса</span>, для это лишь нужно <a <!-- IF {PHP.usr.auth_write} -->href="{PHP|cot_url('folio', 'm=add')}"<!-- ELSE --> href="#AuthModal" uk-toggle<!-- ENDIF --> ><span class="uk-text-bold uk-text-success">добавить свои товары или услуги.</span></a></li>
				<li><span class="uk-button uk-button-warning">ЗАКАЗЧИКУ:</span></li>
				<li>Не хотите ждать, пока появится нужный товар или услуга? Вам стоит в раздел <a class="" href="{PHP|cot_url('projects')}">«{PHP.L.Prj_Depart_Name}»</a>, <a <!-- IF {PHP.usr.auth_write} -->href="{PHP|cot_url('projects', 'm=add')}"<!-- ELSE --> href="#AuthModal" uk-toggle<!-- ENDIF --> ><span class="uk-text-bold uk-text-warning">добавить заявку</span></a> о том, что Вы желаете приобрести конкретный товар или нуждаетесь в конкретных услугах.</li>
			</ul>
		</div>
	<!-- ENDIF -->
	</div>

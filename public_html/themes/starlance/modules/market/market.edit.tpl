...
<section class="uk-container uk-margin-large-top uk-margin-large-bottom">
  <div class="uk-card uk-card-body uk-background-muted uk-border-rounded">
    <header>
      <h3 class="uk-heading-bullet uk-text-uppercase uk-text-bold uk-link-text"> {PHP.L.market_edit_product_title}</h3>
    </header> {FILE "{PHP.cfg.themes_dir}/{PHP.cfg.defaulttheme}/warnings.tpl"} <form action="{PRDEDIT_FORM_SEND}" method="post" name="edit" enctype="multipart/form-data" class="uk-form-stacked uk-margin-large">
      <div uk-grid>
        ...
        <div class="uk-width-1-2@m">
          <div class="uk-margin">
            ...
            <div class="uk-form-controls" uk-tooltip="Выберите категорию">
              <div class="uk-inline uk-width-1-1"> {PRDEDIT_FORM_CAT} </div>
            </div>
          </div>
        </div>
      </div>
      <div class="uk-margin">
        ...
        <div class="uk-form-controls"> {PRDEDIT_FORM_KEYWORDS} </div>
      </div>
      <div class="uk-margin">
        <h5 class="uk-heading-divider uk-text-warning uk-margin-remove">{PHP.L.metatitle}:</h5>
        <label class="uk-form-label uk-margin-remove-top"></label>
        <div class="uk-form-controls"> {PRDEDIT_FORM_METATITLE} </div>
      </div>
      <div class="uk-margin">
        <h5 class="uk-heading-divider uk-text-warning uk-margin-remove">{PHP.L.metadesc}:</h5>
        <label class="uk-form-label uk-margin-remove-top"></label>
        <div class="uk-form-controls"> {PRDEDIT_FORM_METADESC} </div>
      </div>
      <!-- IF {PHP.usr.isadmin} -->
      <div uk-grid>
        <div class="uk-width-1-2@m">
          <div class="uk-margin">
            ...
            <div class="uk-form-controls" uk-tooltip="URL, ЧПУ, - уникальный на весь сайт идентификатор материала">
              <div class="uk-inline uk-width-1-1"> {PRDEDIT_FORM_ALIAS} </div>
            </div>
          </div>
        </div>
        <div class="uk-width-1-2@m">
          <div class="uk-margin">
            ...
            <div class="uk-form-controls">
              <div class="uk-inline uk-width-1-1" uk-tooltip="Разметка для форматирования текстового описания материала"> {PRDEDIT_FORM_PARSER} </div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="uk-margin">
        ...
        <div class="uk-form-controls"> {PRDEDIT_FORM_TEXT} </div>
      </div>
      ...
      <div class="uk-margin">
        ...
        <label class="uk-form-label uk-margin-remove-top"></label>
        <div class="uk-form-controls"> {PRDEDIT_FORM_DEMONSTRATION} </div>
      </div>
      <!-- ENDIF -->
      <!-- IF {PHP.cot_plugins_active.locationselector} -->
      <div class="uk-margin">
        <h4 class="uk-heading-divider uk-text-primary uk-margin-remove">{PHP.L.market_location}:</h4> {PRDEDIT_FORM_LOCATION}
      </div>
      <!-- ENDIF -->
      <!-- BEGIN: TAGS -->
      <div class="uk-margin">
        ...
        <label class="uk-form-label"></label>
        <div class="uk-form-controls">
          <div class="uk-inline uk-width-1-1" uk-tooltip="{PRDEDIT_FORM_TAGS_HINT}"> {PRDEDIT_FORM_TAGS|cot_rc_modify($this, 'class="uk-input"')} </div>
        </div>
      </div>
      <!-- END: TAGS -->
      <div uk-grid>
        <div class="uk-width-1-4@m">
          ...
          <label class="uk-form-label"></label>
          <div class="uk-form-controls uk-position-relative"> {PRDEDIT_FORM_COST} <span class="uk-position-center-right uk-position-small">{PHP.cfg.payments.valuta}</span>
          </div>
        </div>
        <div class="uk-width-expand@m">
          ...
          <div class="uk-margin uk-background-primary uk-tile uk-padding-small uk-border-rounded">
            ...
            <div class="uk-form-controls uk-background-muted uk-padding-small"> {PRDEDIT_FORM_FILE} </div>
          </div>
          <!-- ENDIF -->
        </div>
      </div>
      ...
      <div class="uk-margin">
        ...
        <label class="uk-form-label">
          <span class="uk-text-danger">Только фото и картинки иллюстрирующие ваше предложение. Файлы для продажи прикреплются в форме выше</span>
        </label>
        <!-- ENDIF -->
        <label class="uk-form-label">Расширения <span class="uk-text-danger">{PHP.cfg.plugin.attacher.exts}</span>
        </label>
        <label class="uk-form-label">Макс. размер файлов в байтах <span class="uk-text-danger">{PHP.cfg.plugin.attacher.filesize}</span>
        </label>
        <label class="uk-form-label">Макс количество <span class="uk-text-danger">{PHP.cfg.plugin.attacher.items}</span>
        </label>
        <div class="uk-form-controls"> {PRDEDIT_FORM_ID|att_filebox('market', $this)} </div>
      </div>
      <!-- ENDIF -->
      ...
      <div class="uk-margin">
        ...
        <div class="uk-grid" uk-grid>
          <div class="uk-width-1-2@m"> https://www.youtube.com/watch?v= <span class="uk-text-bold uk-text-primary">w7nB-YC8jc8</span>
          </div>
          <div class="uk-width-1-2@m"> {PRDEDIT_FORM_YOUTUBE_LINK_PRD} </div>
        </div>
      </div>
      <!-- ENDIF -->
      ...
      <div class="uk-margin-top">
        <div class="uk-alert-danger uk-border-rounded" uk-alert>
          ...
          <div class="uk-form-controls uk-margin-small"> {PRDEDIT_FORM_DELETE} </div>
        </div>
      </div>

...
<div class="uk-background-muted uk-section">
	...
	<div class="uk-container uk-container-large">
		...
		<h1 class="uk-heading-bullet uk-text-uppercase uk-text-bold uk-link-text uk-h4 uk-margin-remove-vertical">
			 <!-- IF {PHP.c} -->{CATTITLE}<!-- ELSE --> {PHP.L.market_title} {PHP.cfg.maintitle}<!-- ENDIF -->
		</h1>
		<!-- IF {CATDESC} -->
		<span class="uk-text-muted">{CATDESC}</span>
		<!-- ELSE -->
		<!-- IF {PHP.cfg.market.description} --><span class="uk-text-muted">{PHP.cfg.market.description}</span><!-- ELSE --> <span class="uk-link-text">{PHP.L.market_descr}</span><!-- ENDIF -->
		<!-- ENDIF -->
		</div>
		<div id="listmarket">
				...
				<div class="uk-card uk-background-default">
					...
					<!-- IF {PRD_ROW_ID|att_count('market',$this,'','images')} > 0 -->
						<div data-att-display="all">
							{PRD_ROW_ID|att_display('market',$this,'','attacher.display.marketlist','images',1)}
						</div>
							<!-- ELSE -->
							...
							<!-- ENDIF -->
					<h3 class="uk-heading-bullet uk-margin-remove uk-h5"><a class="uk-link-text" href="{PRD_ROW_URL}" title="{PRD_ROW_SHORTTITLE}" uk-tooltip="{PRD_ROW_SHORTTITLE} - Открыть полную страницу карточки товара"><span>{PRD_ROW_SHORTTITLE}</span></a></h3>
	...
	<!-- ELSE -->
		<div class="uk-card uk-card-body uk-border-rounded uk-background-default uk-animation-scale-up">
			...
			<ul class="uk-list uk-list-divider">
				...
				<li>Вы можете стать первым продавцом в этом отделе <span class="uk-text-bold">онлайн маркетплейса</span>, для это лишь нужно <a <!-- IF {PHP.usr.auth_write} -->href="{PHP|cot_url('market', 'm=add')}"<!-- ELSE --> href="#AuthModal" uk-toggle<!-- ENDIF --> ><span class="uk-text-bold uk-text-success">добавить свои товары или услуги.</span></a></li>

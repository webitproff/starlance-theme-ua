<!-- Если пользователь не из группы АДМИН(5) - шлём его на Аляску! -->
		...
		<nav class="uk-navbar-height uk-navbar-container uk-container uk-container-large uk-navbar-transparent" uk-navbar>
			...
			<div class="uk-navbar-left uk-visible@m">
				...
				<ul class="uk-navbar-nav uk-light">
					...
					<!-- ENDIF -->
					<li>
						<a href="#">{PHP.L.Users}</a>
						<div class="uk-navbar-dropdown uk-border-rounded">
							<ul class="uk-nav uk-navbar-dropdown-nav">
							<!-- IF {PHP.cot_groups.2.id} -->
								<li><a class="" href="{PHP.cot_groups.2.alias|cot_url('users', 'group='$this)}">{PHP.cot_groups.2.name}</a></li>
							<!-- ENDIF -->
							<!-- IF {PHP.cot_groups.3.id} -->
								<li><a class="" href="{PHP.cot_groups.3.alias|cot_url('users', 'group='$this)}">{PHP.cot_groups.3.name}</a></li>
							<!-- ENDIF -->
							<!-- IF {PHP.cot_groups.4.id} -->
								<li><a class="" href="{PHP.cot_groups.4.alias|cot_url('users', 'group='$this)}">{PHP.cot_groups.4.name}</a></li>
							<!-- ENDIF -->
							<!-- IF {PHP.cot_groups.5.id} -->
								<li><a class="" href="{PHP.cot_groups.5.alias|cot_url('users', 'group='$this)}">{PHP.cot_groups.5.name}</a></li>
							<!-- ENDIF -->
							<!-- IF {PHP.cot_groups.6.id} -->
								<li><a class="" href="{PHP.cot_groups.6.alias|cot_url('users', 'group='$this)}">{PHP.cot_groups.6.name}</a></li>
							<!-- ENDIF -->
							<!-- IF {PHP.cot_groups.7.id} -->
								<li><a class="" href="{PHP.cot_groups.7.alias|cot_url('users', 'group='$this)}">{PHP.cot_groups.7.name}</a></li>
							<!-- ENDIF -->
							</ul>
						</div>
					</li>
					...
					<!-- ENDIF -->
					<a class="uk-button uk-button-small uk-button-warning" href="{PHP|cot_url('admin')}" uk-tooltip="{PHP.L.Adminpanel}"><span uk-icon="happy" class="uk-margin-small-right uk-icon"></span><span  class="uk-text-middle">{PHP.L.Adminpanel}</span></a>
				</ul>
			</div>
			<div class="uk-navbar-right">
				...
				<!-- ENDIF -->
						<!-- IF {PHP.out.notices} -->
						<a class="uk-light" href="#">Есть сообщения</a>
						<div class="uk-navbar-dropdown uk-border-rounded">
							<ul class="uk-nav uk-list uk-list-divider uk-navbar-dropdown-nav">
								<li><span class="uk-link-text uk-text-middle">{PHP.out.notices}</span></li>
							</ul>
						</div>
						<!-- ENDIF -->
...
<div id="User-Account" uk-offcanvas="flip: true; overlay: true">
    <div class="uk-offcanvas-bar">
        ...
        <div class="uk-panel uk-margin-remove-top">
			...
			<span uk-icon="users" class="uk-margin-small-right uk-icon"></span><span class="uk-visible@s uk-text-middle uk-text-success">{PHP.L.Maingroup}:</span> <span class="uk-text-middle uk-text-warning">{PHP.cot_groups.5.name}</span>
			</p>
			<hr class="uk-divider-icon uk-margin-remove">
        </div>	
        <ul class="uk-nav uk-nav-default">
			<ul class="uk-nav-parent-icon" uk-nav>
				<li class="uk-parent">
					...
					<ul class="uk-nav-sub uk-list uk-list-divider">
						<li><a class="uk-color-f5" href="{PHP|cot_url('admin')}" target="_blank">{PHP.L.Adminpanel}</a></li>
			...
			<!-- IF {PHP.cot_modules.payments} AND {PHP.cfg.payments.balance_enabled} -->			
				<li class="uk-parent">
					<a class="" href="#"><img class="uk-margin-small-right" width="27" height="27" alt="" src="{PHP.cfg.mainurl}/{PHP.cfg.themes_dir}/{PHP.cfg.defaulttheme}/img/icon/wallet.svg" uk-svg="uk-preserve"><span class="uk-text-uppercase uk-text-middle">Финансы:</span> <span class="uk-text-success" uk-tooltip="{PHP.L.payments_mybalance}">{HEADER_USER_BALANCE|number_format($this, '2', '.', ' ')} {PHP.cfg.payments.valuta}</span></a>
					<ul class="uk-nav-sub uk-list uk-list-divider">
			...
			<!-- IF {PHP.cot_modules.market} -->		
				<li class="uk-parent">
					<a class="" href="#"><img class="uk-margin-small-right" width="27" height="27" alt="" src="{PHP.cfg.mainurl}/{PHP.cfg.themes_dir}/{PHP.cfg.defaulttheme}/img/icon/my-ecommerce.svg" uk-svg="uk-preserve"><span class="uk-text-lgrey uk-text-uppercase uk-text-middle">{PHP.L.Prd_My_Marketplace}</span> </a>
					<ul class="uk-nav-sub uk-list uk-list-divider">
						<li><a class="uk-color-f5" href="{PHP.usr.id|cot_url('users', 'm=details&id=$this&tab=market')}" data-offset="50">{PHP.L.market_myproducts}</a></li>
						<!-- IF {PHP.cot_plugins_active.marketorders} -->
						<li><a class="uk-color-f5" href="{PHP|cot_url('marketorders', 'm=sales')}">{PHP.L.marketorders_mysales}</a></li>
						<li><a class="uk-color-f5" href="{PHP|cot_url('marketorders', 'm=purchases')}">{PHP.L.marketorders_mypurchases}</a></li>
						<!-- ENDIF -->
						<li><a class="uk-color-f5" href="{PHP|cot_url('market', 'm=add')}">{PHP.L.market_add_product}</a></li>
					</ul>
				</li>
				<li class="uk-nav-divider"></li>			
			<!-- ENDIF -->
			<!-- IF {PHP.cot_modules.projects} -->			
				<li class="uk-parent">
					...
					<ul class="uk-nav-sub uk-list uk-list-divider">
						...
						<li><a class="uk-color-f5" href="{PHP|cot_url('projects', 'm=useroffers')}">{PHP.L.offers_useroffers}</a></li>
						<!-- ENDIF -->
						<!-- IF {PHP.cot_plugins_active.sbr} -->
						<li><a class="uk-color-f5" href="{PHP|cot_url('sbr')}">{PHP.L.sbr_mydeals}</a></li>
						<!-- ENDIF -->
						<!-- IF {PHP.cot_plugins_active.sbr} -->
						<li><a class="uk-color-f5" href="{PHP|cot_url('admin', 'm=other&p=sbr')}" target="_blank">Адм. «{PHP.L.sbr}»</a></li>
						<!-- ENDIF -->
			...
			<!-- IF {PHP.cot_modules.folio} -->			
				<li class="uk-parent">
					<a class="" href="#"><span class="uk-text-lgrey uk-text-uppercase">{PHP.L.Folio_My_Menu_Name}</span> </a>
					<ul class="uk-nav-sub uk-list uk-list-divider">
						<li><a class="uk-color-f5" href="{PHP.usr.id|cot_url('users', 'm=details&id=$this&tab=portfolio')}" data-offset="50">{PHP.L.Folio_My_List_dtls}</a></li>
						<li><a class="uk-color-f5" class="" href="{PHP|cot_url('folio', 'm=add')}">{PHP.L.Folio_My_Add_List}</a></li>
					</ul>
				</li>
				<li class="uk-nav-divider"></li>
			<!-- ENDIF -->
			<!-- IF {PHP.cot_plugins_active.sbr} -->
				<li><a class="uk-color-f5" href="{PHP|cot_url('admin', 'm=other&p=sbr')}"><img class="uk-margin-small-right" width="27" height="27" alt="" src="{PHP.cfg.mainurl}/{PHP.cfg.themes_dir}/{PHP.cfg.defaulttheme}/img/icon/036-policeman.svg" uk-svg="uk-preserve"><span class="uk-text-middle">Управление «{PHP.L.sbr}»</span></a></li>
				<li><a class="uk-color-f5" href="{PHP|cot_url('sbr')}"><img class="uk-margin-small-right" width="27" height="27" alt="" src="{PHP.cfg.mainurl}/{PHP.cfg.themes_dir}/{PHP.cfg.defaulttheme}/img/icon/022-shield-1.svg" uk-svg="uk-preserve"><span class="uk-text-middle uk-text-uppercase">{PHP.L.sbr_mydeals}</span></a></li>
				<li class="uk-nav-divider"></li>
			<!-- ENDIF -->
			</ul>
        </ul>
